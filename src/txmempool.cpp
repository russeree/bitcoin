...
static CTxMemPool::Options&& Flatten(CTxMemPool::Options&& opts, bilingual_str& error)
{
    ...
    // The mempool must have room for at least a handful of maximum size
    // clusters. 5x the (much larger) cluster size limit approximately
    // preserves the historical ~5 MB minimum.
    int64_t cluster_limit_bytes = opts.limits.cluster_size_vbytes * 5;
